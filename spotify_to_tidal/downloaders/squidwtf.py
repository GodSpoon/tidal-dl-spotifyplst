...
def _stream_to_disk(url: str, dest: Path, session: requests.Session) -> None:
...
def _fetch_album_tracks(album_id: str, session: requests.Session) -> list[dict[str, Any]]:
    ...
    return payload["data"].get("tracks", {}).get("items", [])
...
def _download_album(
    ...
    for tr in tracks:
        ...
        results.append(_download_track(sub_task, quality, out_dir, session))
    return results
...
class SquidWtfDownloader(DownloaderBackend):
    ...
    def download(
        ...
        **kwargs: Any,
        ...
        print(f"[squidwtf] Quality: {quality_label} (Qobuz format_id={quality}, ext={_ext_for_quality(quality)})")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            for task in plan:
                if task["type"] == "album":
                    fut = pool.submit(_download_album, task, quality, out_dir, session)
                else:
                    fut = pool.submit(_download_track, task, quality, out_dir, session)
                futs[fut] = task
